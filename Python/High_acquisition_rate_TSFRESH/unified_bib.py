...
def Classification_it (X_train,X_test,y_train,y_test): #Classifiers
    ...
    for i, (name, clf) in enumerate(zip(names, classifiers)):
        ...
        score = clf.score(X_test, y_test)
        y_predict = list(clf.predict(X_test))
        Accuracy[i] = score*100
        Precision[i] = precision_score(y_test, y_predict,zero_division=0)*100
        Recall[i] = recall_score(y_test, y_predict,zero_division=0)*100
        F1[i] = f1_score(y_test, y_predict,zero_division=0)*100
                
    results = []

    #Calculinng Mean and Std. Derivation 
    for i in range(len(names)):
        results.append(['{:.2f}%'.format(Accuracy[i]),
