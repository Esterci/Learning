...
def main(n_i,total, background_percent, test_size, dat_set_percent):
    ...
    att_list = ['"dirichlet_distribution"', 'tied', 'diag', 'spherical']
    ...
    for covariance_type in att_list:
        ...
        prob = gmm.predict_proba()
        
        np.savetxt('results_99/posterior_prob_GMEM_' + covariance_type + '_' + 
    ...
    for prior in att_list:
        print('=> Iteration Number {}:'.format(n_i+1),'              - BGM \'' +
                                                    covariance_type + '\'')
        gmm = mixture.BayesianGaussianMixture(n_components=5,
                            weight_concentration_prior_type=prior.fit(training_data)

        prob = gmm.predict_proba()
        
        np.savetxt('results_99/posterior_prob_BGM_' + covariance_type + '_' + 
                    str(n_i) +'.csv', prob, delimiter=',')                
